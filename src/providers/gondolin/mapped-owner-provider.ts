...
function mapStats<T extends { uid: number; gid: number; mode: number }>(
  stats: T,
  uid: number,
  gid: number,
): T {
...
function mapHandleStats(
  ...
  return {
    ...handle,
    stat: async (options?: object) =>
      mapStats(await handle.stat(options), uid, gid),
    statSync: (options?: object) =>
      mapStats(handle.statSync(options), uid, gid),
  };
}
...
export class MappedOwnerProvider implements VirtualProvider {
  ...
  async lstat(
    ...
    return mapStats(
      await this.backend.lstat(path, options),
      this.uid,
      this.gid,
    );
  }
  ...
  readdir(
    path: string,
    options?: object,
  ): Promise<Array<string | import("node:fs").Dirent>> {
  ...
  readdirSync(
    path: string,
    options?: object,
  ): Array<string | import("node:fs").Dirent> {
  ...
  readFile(
    ...
    if (!this.backend.readFile)
      return this.backend.open(path, "r").then((h) => h.readFile(options));
  ...
  readFileSync(
    ...
    if (!this.backend.readFileSync)
      return this.backend.openSync(path, "r").readFileSync(options);
  ...
  writeFile(
    ...
    if (!this.backend.writeFile)
      return this.backend
        .open(path, "w", options?.mode)
        .then((h) => h.writeFile(data, options));
  ...
  writeFileSync(
    ...
    if (!this.backend.writeFileSync)
      return this.backend
        .openSync(path, "w", options?.mode)
        .writeFileSync(data, options);
    return this.backend.writeFileSync(path, data, options);
  }
  ...
  appendFileSync(
    ...
    if (!this.backend.appendFileSync)
      return this.backend
        .openSync(path, "a", options?.mode)
        .writeFileSync(data, options);
    return this.backend.appendFileSync(path, data, options);
  }
  ...
  copyFileSync(src: string, dest: string, mode?: number): void {
    if (!this.backend.copyFileSync)
      throw new Error("copyFileSync not supported");
    return this.backend.copyFileSync(src, dest, mode);
  }
  ...
  readlinkSync(path: string, options?: object): string {
    if (!this.backend.readlinkSync)
      throw new Error("readlinkSync not supported");
